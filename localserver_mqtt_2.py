...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        if data_json["hostaddress"] == SignRouterA:
            ...
            if cursor == None:
                DBclient.xljy.sign_table.insert({"mac":data_json["data"][0]["address"], "rssi0":data_json["rssi"], "rssi1":0, "time":data_json["routertime"]})
            else:
                if data_json["routertime"] - cursor["time"] < THRESHOLD_TIME:
                    ...
                    if cursor["rssi1"] != 0:
                        log.debug("get a full data")
                        ret = isInSchool(data_json["rssi"], cursor["rssi1"])
                        if ret == 0:
                    ...
                    DBclient.xljy.sign_table.update_one({"mac":data_json["data"][0]["address"]}, {"$set":{"rssi0":data_json["rssi"], "rssi1":0, "time":data_json["routertime"]}})
        elif data_json["hostaddress"] == SignRouterB:
            cursor = DBclient.xljy.sign_table.find_one({"mac":data_json["data"][0]["address"]})
            if cursor == None:
                DBclient.xljy.sign_table.insert({"mac":data_json["data"][0]["address"], "rssi0":0, "rssi1":data_json["rssi"], "time":data_json["routertime"]})
            else:
                if data_json["routertime"] - cursor["time"] < THRESHOLD_TIME:
                    res = DBclient.xljy.sign_table.update_one({"mac":data_json["data"][0]["address"]}, {"$set":{"rssi1":data_json["rssi"]}})
                    if cursor["rssi0"] != 0:
                        log.debug("get a full data")
                        ret = isInSchool(cursor["rssi0"],data_json["rssi"])
                        if ret == 0:
                            CheckDo(data_json["routertime"], data_json["data"][0]["address"], 1)
                        elif ret == 1:
                            CheckDo(data_json["routertime"], data_json["data"][0]["address"], 0)
                    ...
                    DBclient.xljy.sign_table.update_one({"mac":data_json["data"][0]["address"]}, {"$set":{"rssi1":data_json["rssi"], "rssi0":0, "time":data_json["routertime"]}})
